...
def run_simulation(input_file, root_dir, cr, c, model_name):
    grid = separation_simulator_grid_loader(input_file)
    cs = SeparationSimulator(grid, c[0], c[1], cr)
...
if __name__ == "__main__":
    ...
    root_dir = "output/SeparationSimulator/10-31-multiclass10-7perclass/"
    ...
    for f in input_files:
        ...
        #connectivity_rules = [ConnectivityRule.Interclass, ConnectivityRule.Strict]
        connectivity_rules = [ConnectivityRule.Strict]
        ...
        for cr in connectivity_rules:
            ...
            for c in itertools.product(compression_constants, separation_constants):
                threading.Thread(target=run_simulation, args=(input_file, root_dir, cr, c, model_name)).start()
