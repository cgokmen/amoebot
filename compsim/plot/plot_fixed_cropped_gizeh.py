...
class FixedCroppedVectorPlotter(object):
    ...
    def draw_plot(self, path):
        ...
        self.max_x_dist = max(self.max_x_dist, max_coords[0])
        self.max_y_dist = max(self.max_y_dist, max_coords[1])
        ...
        return (path, group.translate(xy=(-1 * com_coords)))

    def plot(self, filename):
        self.plots.append(self.draw_plot(os.path.join(self.path, filename)))

    def close(self):
        # Write all of the plots
        w, h = 2 * (self.max_x_dist + CORNER_PADDING), 2 * (self.max_y_dist + CORNER_PADDING)

        for path, group in self.plots:
            plt = PDFS(path, w, h)
