...
from . import CompressionSimulator, Directions, Ant, Food
...
class TokenPassingSimulator(CompressionSimulator):
    ...
    def activate(self, particle):
        ...
        if particle.food_direction is None and len(food_nbrs) > 0:
            direction = Directions.ALL[next(i for i, j in enumerate(food_nbrs) if j is not None)]
        ...
        if particle.token is not None and (not particle.token.is_food) and len(food_nbrs) > 0:
            ...
            particle.token.direction = particle.token.Directions.shift_counterclockwise_by(direction,
                1)  # Note that this differs from the pseudocode since the directions increase in the opposite direction here
        ...
        if particle.token is not None and particle.token.is_food:
            ...
            if nbr.token is None:
                particle.token.direction = Directions.shift_counterclockwise_by(d, 3)
    ...
    @staticmethod
    def get_next_neighbor(neighbors, direction, counterclockwise):
        add = 1 if counterclockwise else -1

        current_direction = direction

        while True:
            current_direction = Directions.shift_counterclockwise_by(current_direction, add)
